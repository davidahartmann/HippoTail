...
for fi = 8:length(mf)
    ...
    for i = 1:length(r)
        ...
        for ib = 1:length(block_name)
            ...
            load(fullfile(CCEP_data_folder, [block, '_CCEP.mat'])); % read as CCEP
...
%             plot_CCEP_SpecChan(CCEP,sbj, idx, 1, outdir)
            CCEPwave = permute(CCEP.wave, [3,1,2]);
            CCEPwave = reshape((CCEPwave(:,:,idx)), 2201, []);
            CCEPwave_all = [CCEPwave_all CCEPwave];
