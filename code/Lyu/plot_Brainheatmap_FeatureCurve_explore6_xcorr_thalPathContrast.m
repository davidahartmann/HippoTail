...
% comparisons between F1-inflow (bilateral) vs. F3-outflow (bilateral)
...
metaT = readtable(fullfile(result_folder, 'table_CCEPnewpipOutput_wholebrain_anatomical_info_activationRedone4.csv'));
...
% zscore data to make them more comparable
allr_inflow = metaT.peak_maxCor_clst1(ismember(metaT.JP_label_in, ROIs));
allr_outflow = metaT.peak_maxCor_clst3(ismember(metaT.JP_label_out, ROIs));
metaT.zr_inflow = nan(size(metaT,1),1);
metaT.zr_outflow = nan(size(metaT,1),1);
metaT.zr_inflow(ismember(metaT.JP_label_in, ROIs)) = zscore(allr_inflow, 0,'omitnan');
metaT.zr_outflow(ismember(metaT.JP_label_out, ROIs)) = zscore(allr_outflow, 0,'omitnan');
...
for ix = 3%:length(xHEMIs)
    ...
    for ip = 1:length(paths)
        ...
        if strcmp(pathway, 'inflow')
            nF = 1;
        elseif strcmp(pathway, 'outflow')
            nF = 3;
        end
        % use zscore for pathway conparisons
        featurename = sprintf('zr_%s', pathway);
        % coordinates
        ...
        metaStruct(ip).weights = weights_cell;
        metaStruct(ip).jplabel = jplabel_cell;
        metaStruct(ip).coordinates = coordinates_cell;
    end % for ip
    ...
    cfg.maxActRadius       = 250;
    cfg.anotFontSize       = 15;
    cfg.titleFontSize      = 17;
    cfg.titletext          = {[strjoin(contrast_btwn, ' vs. ')],''};
    ...
    cfg.cmaps = flipud(cbrewer2('div','RdBu',128, 'spline'));%colr.cm; %
    ...
    %% contast between inflow and outflow for each ROI
    r_coordinates_cell = cell(3,1); % each cell correspond to each ROI
    r_weights_cell     = cell(3,1);
    r_jplabel_cell     = cell(3,1);
    
    for ir = 1:length(anot)
        % paths{ip1} vs. paths{ip2}
        ip1 = find(strcmp(paths, contrast_btwn{1}));
        ip2 = find(strcmp(paths, contrast_btwn{2}));
        
        [common_coords, i1,i2] = ...
            intersect(metaStruct(ip1).coordinates{ir}, ...
            metaStruct(ip2).coordinates{ir}, 'rows');
        r_coordinates_cell{ir} = common_coords;
        r_weights_cell{ir} =...
            metaStruct(ip1).weights{ir}(i1) - metaStruct(ip2).weights{ir}(i2);
        r_jplabel_cell{ir} = metaStruct(ip1).jplabel{ir}(i1);
    end
    ...
    %             cfg.clims = repmat([-1*smallerlimit  smallerlimit], 3, 1);
     cfg.clims = [-1.5 1.5];
    %--------------------------------------------------------------------------------------------------
    plot_folder = fullfile(output_folder, 'Path_Contrasts');
    if ~exist(plot_folder, 'dir'); mkdir(plot_folder); end
    fname = sprintf('BrainHeatmap_thalamusDiv_feature%d_%s_%s_%s_thalPathContrasts_withCbar_RdBu_FSLR.pdf', nF, pathway, featurename,xHEMI);
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    plotBrainSurfaceWeight(cmcortex , r_coordinates_cell, r_weights_cell, [], r_jplabel_cell, plot_folder, anot, pers_rule, cfg, fname);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
